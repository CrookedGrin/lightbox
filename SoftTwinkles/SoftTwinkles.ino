...
#define DENSITY         255
#define LEDS_PER_CYCLE  7 // higher == more light up each time

void setup() {
  FastLED.addLeds<OCTOWS2811,COLOR_ORDER>(leds, NUM_LEDS_PER_STRIP).setCorrection(TypicalLEDStrip);
...
void softtwinkles() {
  ...
  // brighter over time.
  if( random8() < DENSITY) {
    for ( int j=0; j < LEDS_PER_CYCLE; j++) {
      int idx = random16(NUM_LEDS);
      if( !leds[idx] ) leds[idx] = lightcolor;
    }
  }
}
