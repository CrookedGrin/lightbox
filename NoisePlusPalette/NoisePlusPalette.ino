...
void setup() {
  delay(3000);
  LEDS.addLeds<OCTOWS2811,COLOR_ORDER>(leds, NUM_LEDS_PER_STRIP);
...
{
  //TODO: This way of setting duration and picking which pallet to use is fucking stupid.
  uint8_t secondHand = ((millis() / 1000) / HOLD_PALETTES_X_TIMES_AS_LONG) % 60;
  static uint8_t lastSecond = 99;
  
  if( lastSecond != secondHand) {
    lastSecond = secondHand;
    if( secondHand ==  0)  { currentPalette = RainbowColors_p;         speed = 10; scale = 40; colorLoop = 1; }
    if( secondHand == 15)  { currentPalette = CloudColors_p;           speed =  4; scale = 30; colorLoop = 0; }
    if( secondHand == 30)  { currentPalette = LavaColors_p;            speed =  8; scale = 50; colorLoop = 0; }
    if( secondHand == 45)  { currentPalette = PartyColors_p;           speed = 15; scale = 30; colorLoop = 1; }
//    if( secondHand ==  5)  { SetupPurpleAndGreenPalette();             speed = 10; scale = 50; colorLoop = 1; }
    if( secondHand == 55)  { SetupBlackAndWhiteStripedPalette();       speed = 20; scale = 30; colorLoop = 1; }
